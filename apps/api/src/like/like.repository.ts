...
export const likeRepository = {
    ...
    likeImage: (userId: string, imageId: string) =>
        prisma.like.create({ data: { userId, imageId } }),

    unlike: (id: string) => prisma.like.delete({ where: { id } }),

    countByGallery: (galleryId: string) =>
        prisma.like.count({ where: { galleryId } }),

    countByImage: (imageId: string) =>
        prisma.like.count({ where: { imageId } }),
};
